...
std::unique_ptr<Activity> AppsMenuActivity::createCategoryActivity(int selectorIndex) {
  ...
  switch (selectorIndex) {
      ...
      case 5: {
        std::vector<AppCategoryActivity::AppEntry> e = {
            ...
            {"8-Ball Pool", "1v1 pool duel: phones + spectators", UIIcon::Hotspot, [](GfxRenderer& r, MappedInputManager& m) { return std::make_unique<PoolDuelActivity>(r, m); }},
        };
...
void AppsMenuActivity::loop() {
  ...
  if (SETTINGS.uiTheme == CrossPointSettings::UI_THEME::RADAR) {
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
      std::unique_ptr<Activity> app = createCategoryActivity(selectorIndex);
...
void AppsMenuActivity::drawTile(int index, int x, int y, int w, int h, bool selected) const {
  ...
  switch (index) {
    ...
    case 5: name = "GAMES";    subtitle = "Entertainment";      appCount = 13; break;
