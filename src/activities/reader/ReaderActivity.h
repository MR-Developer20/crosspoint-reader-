...
class ReaderActivity final : public Activity {
  ...
  // Converts a .md to a synthetic EPUB (first open, with the indexing popup) and returns an
  // Epub over the generated book.epub; also non-static for the popup/framebuffer loan.
  std::unique_ptr<Epub> loadMarkdown(const std::string& path);
