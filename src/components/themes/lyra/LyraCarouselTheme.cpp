...
void LyraCarouselTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect,
  ...
  auto drawCover = [&](int bookIdx, int x, int y, int maxW, int maxH) -> bool {
    ...
    if (!hasCover) {
      ...
      renderer.fillRoundedRect(x, y + maxH / 3, maxW, 2 * maxH / 3, kCornerRadius, /*roundTopLeft=*/false,
                               ...
                               Color::Black);
      renderer.drawIcon(CoverIcon, x + maxW / 2 - 16, y + 8, 32);
    }
    return true;
  };
  ...
  if (!coverRendered) {
    ...
    // Border only drawn if a book exists in the slot.
...
void LyraCarouselTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = 0; i < buttonCount; ++i) {
    ...
    if (rowIcon != nullptr) {
      ...
      if (bmp != nullptr) {
        if (selected)
          renderer.drawIconInverted(bmp, iconX, iconY, kMenuIconSize);
        else
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    if (rowIcon != nullptr) {
      ...
      if (iconBitmap != nullptr) {
        ...
        if (sel)
          renderer.drawIconInverted(iconBitmap, ix, itemY + iconY, iconSize);
        else
          renderer.drawIcon(iconBitmap, ix, itemY + iconY, iconSize);
      }
    }
    ...
    if (!valueText.empty()) {
      if (sel && highlightValue) {
        renderer.fillRoundedRect(
            ...
            valueWidth + hPad, rowHeight, kCornerRadius, Color::Black);
      }
...
// ---------------------------------------------------------------------------
// Tab bar — solid black active tab with inverted text when focused
// ---------------------------------------------------------------------------
